...
render_context_3d_renderbuffer::render_context_3d_renderbuffer() {
    related_value_ = new uint32_t();
}

render_context_3d_renderbuffer::~render_context_3d_renderbuffer() {
    render_context_3d_renderbuffer::dispose();
}

bool render_context_3d_renderbuffer::is_useable() {
    return _builded_ == status_ 
        && related_ && related_->is_useable();
}

bool render_context_3d_renderbuffer::dispose() {
    ...
    struct __task__ {
        ...
        void run(barrel_buffer*) {
            ...
            if (context_) {
                ...
                context_->data_->delete_render_buffers(1, v_);
            }
            delete v_;
        }
    };
