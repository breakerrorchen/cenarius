...
render_context_3d_buffer::render_context_3d_buffer() {
    related_value_ = new uint32_t();
}

render_context_3d_buffer::~render_context_3d_buffer() {
    render_context_3d_buffer::dispose();
}

bool render_context_3d_buffer::is_useable() {
    return _builded_ == status_ 
        && related_ && related_->is_useable();
}

bool render_context_3d_buffer::dispose() {
    ...
    struct __task__ {
        ...
        void run(barrel_buffer*) {
            ...
            if (context_) {
                ...
                context_->data_->delete_buffers(1, v_);
            }
            delete v_;
        }
    };
