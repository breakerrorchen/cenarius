 ...
 *      This value is clamped to the range 0 to 2n -1 where n is 
 *      the number of bitplanes in the stencil buffer. 
 *      The default value is 0.
 *
 * mask
 *      A GLuint specifying a bit-wise mask that is used to AND 
 *      the reference value and the stored stencil value when the 
 *      test is done. The default value is all 1.
 */
void render_context_3d::stencil_func(js_parameter& _parameter) {
    ...
    uint32_t func = (uint32_t)_0.to_int32();
    int      ref  = (int)     _1.to_int32();
    uint32_t mask = (uint32_t)_2.to_int32();
    ...
    struct __task__ {
        ...
        uint32_t func_; int ref_; uint32_t mask_;
        void run(barrel_buffer*) {
            assert(context_ && context_->data_);
            auto raw_context = context_->data_;
            assert(raw_context);
            raw_context->stencil_func(func_, ref_, mask_);
        }
    };
