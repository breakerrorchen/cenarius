 ...
 *
 * zpass
 *      A GLenum specifying the function to use when both the stencil test and 
 *      the depth test pass, or when the stencil test passes and there is no 
 *      depth buffer or depth testing is disabled. The default value is gl.KEEP.
 */
void render_context_3d::stencil_op(js_parameter& _parameter) {
    ...
    auto fail   = (uint32_t)_0.to_int32();
    auto z_fail = (uint32_t)_1.to_int32();
    auto z_pass = (uint32_t)_2.to_int32();
    ...
    struct __task__ {
        ...
        uint32_t fail_ = 0, z_fail_ = 0, z_pass_ = 0;
        void run(barrel_buffer*) {
            assert(context_ && context_->data_);
            auto raw_context = context_->data_;
            assert(raw_context);
            raw_context->stencil_op(fail_, z_fail_, z_pass_);
        }
    };
