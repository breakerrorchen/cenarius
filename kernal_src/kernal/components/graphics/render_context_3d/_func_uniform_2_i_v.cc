 ...
 */
void render_context_3d::uniform_2_i_v(js_parameter& _parameter) {
    ...
    struct __task__ {
        ...
        int32_t location_; int32_t v0_, v1_;
        void run(barrel_buffer*) {
            assert(context_ && context_->data_);
            auto raw_context = context_->data_;
            assert(raw_context);
            raw_context->uniform_2_i(location_, v0_, v1_);
        }
    };
