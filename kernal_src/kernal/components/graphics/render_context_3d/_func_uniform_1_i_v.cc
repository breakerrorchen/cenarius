 ...
 */
void render_context_3d::uniform_1_i_v(js_parameter& _parameter) {
    ...
    if (!v.is_typed_arr_int32()) return;
    auto typedarr_buffer = v.typedarr_buffer();
    if (typedarr_buffer.size<int32_t>() < 1) return;
    auto value = typedarr_buffer.at<int32_t>(1);
    if (DWL_FLOAT == local_uniform->type_) {
        local_uniform->value_.f_16_[0] = value;
    } else if (DWL_BOOL == local_uniform->type_) {
        local_uniform->value_.b_16_[0] = (bool)value;
    } else if (DWL_INT == local_uniform->type_) {
        local_uniform->value_.i_16_[0] = (int)value;
    }
    ...
    struct __task__ {
        ...
        void run(barrel_buffer*) {
            ...
            assert(raw_context);
            raw_context->uniform_1_i_v(location_, 1, &value_);
        }
    };
