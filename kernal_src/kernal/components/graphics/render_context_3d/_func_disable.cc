 ...
 */
void render_context_3d::disable(js_parameter& _parameter) {
    ...
    if (DWL_BLEND == value) {
        context_cache_.enable_blend_                      = false;
    } else if (DWL_CULL_FACE == value) {
        context_cache_.enable_cull_face_                  = false;
    } else if (DWL_DEPTH_TEST == value) {
        context_cache_.enable_depth_test_                 = false;
    } else if (DWL_DITHER == value) {
        context_cache_.enable_dither_                     = false;
    } else if (DWL_POLYGON_OFFSET_FILL == value) {
        context_cache_.enable_polygon_offset_fill_        = false;
    } else if (DWL_SAMPLE_ALPHA_TO_COVERAGE == value) {
        context_cache_.enable_sample_alpha_to_coverage_   = false;
    } else if (DWL_SAMPLE_COVERAGE == value) {
        context_cache_.enable_coverage_                   = false;
    } else if (DWL_SCISSOR_TEST == value) {
        context_cache_.enable_scissor_test_               = false;
    } else if (DWL_STENCIL_TEST == value) {
        context_cache_.enable_stencil_test_               = false;
    } else {
        return;
    }
