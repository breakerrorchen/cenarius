...
void render_context_2d::shadow_offset_y(js_parameter& _parameter) {
    ...
    _parameter.set_return(
        js_value::create(js_context, context.shadow_offset_y_));
}

void render_context_2d::set_shadow_offset_y(js_parameter& _parameter) {
    assert(related_ && !related_->lt_context_stack_.empty());
    if (_parameter.get_argument_count() <= 0) return;
    auto shadow_offset = _parameter[0].to_float();
    auto& context = related_->lt_context_stack_.top();
    context.shadow_offset_y_ = shadow_offset;
    struct __task__ {
        ...
        void run(barrel_buffer*) {
            ...
            context.shadow_offset_y_ = shadow_offset_;
        }
    };
