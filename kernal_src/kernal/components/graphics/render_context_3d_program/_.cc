...
render_context_3d_program::render_context_3d_program() {
    related_value_ = new uint32_t();
}

render_context_3d_program::~render_context_3d_program() {
    render_context_3d_program::dispose();
}

bool render_context_3d_program::is_useable() {
    return _builded_ == status_ 
        && related_ && related_->is_useable();
}

bool render_context_3d_program::is_deleted() {
    return _deleted_ == status_;
}

bool render_context_3d_program::dispose() {
    ...
    struct __task__ {
        ...
        void run(barrel_buffer*) {
            ...
            if (context_) {
                ...
                context_->data_->delete_program(*v_);
            }
            delete v_;
        }
    };
