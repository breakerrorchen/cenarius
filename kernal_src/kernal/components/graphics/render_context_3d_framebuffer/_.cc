...
render_context_3d_framebuffer::render_context_3d_framebuffer() {
    related_value_ = new uint32_t();
}

render_context_3d_framebuffer::~render_context_3d_framebuffer() {
    render_context_3d_framebuffer::dispose();
}

bool render_context_3d_framebuffer::is_useable() {
    return _builded_ == status_ 
        && related_ && related_->is_useable();
}

bool render_context_3d_framebuffer::dispose() {
    ...
    struct __task__ {
        ...
        void run(barrel_buffer*) {
            ...
            if (context_) {
                ...
                context_->data_->delete_frame_buffers(1, v_);
            }
            delete v_;
        }
    };
